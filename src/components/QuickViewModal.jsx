...
export default function QuickViewModal({ media, type = 'movie', isOpen, onClose }) {
  ...
  return (
    ...
    >
      <div 
        className="relative w-full max-w-2xl bg-white border border-black/10 rounded-3xl overflow-hidden shadow-2xl animate-slide-up"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Outlined Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-40 p-2 rounded-full bg-white/90 hover:bg-white text-[#09090B] border border-black/10 backdrop-blur-md transition cursor-pointer shadow-sm"
        ...
        <div className="relative h-60 sm:h-72 w-full bg-black overflow-hidden flex items-center justify-center">
          ...
          {trailerKey && (
            <button
              ...
              className="absolute bottom-4 right-4 z-30 p-2 rounded-full bg-white/90 hover:bg-white text-[#09090B] border border-black/10 backdrop-blur-md transition cursor-pointer shadow-sm"
          ...
          <div className="absolute top-4 left-4 z-20">
            <span className="px-2.5 py-0.5 rounded-full bg-[#09090B] text-white text-[10px] font-mono uppercase tracking-wider font-semibold shadow-sm">
              {mediaType}
            </span>
          </div>
        </div>
        ...
        <div className="p-6 sm:p-7 space-y-5">
          <div className="space-y-1.5">
              ...
              {currentData.vote_average > 0 && (
                <>
                  <span className="flex items-center gap-1 text-[#09090B] font-bold">
                    <Star className="w-3 h-3 text-[#2563EB] fill-[#2563EB] stroke-[1.5]" /> {currentData.vote_average.toFixed(1)}
                  </span>
                  <span>·</span>
                </>
              )}
            ...
            <h2 className="text-2xl sm:text-3xl font-bold text-[#09090B] font-['Outfit'] tracking-tight">
            ...
            {currentData.genres && currentData.genres.length > 0 && (
              <div className="flex flex-wrap gap-1.5 pt-1">
                {currentData.genres.slice(0, 4).map((g) => (
                  <span key={g.id} className="px-2 py-0.5 rounded-full bg-black/[0.04] border border-black/[0.08] text-[10px] font-mono text-[#52525B]">
                    {g.name}
                  </span>
                ))}
              </div>
          ...
          <p className="text-[#52525B] text-xs sm:text-sm leading-relaxed line-clamp-3 font-normal">
            {currentData.overview || "No synopsis available for this selection."}
          </p>

          {/* Cast */}
          {details?.credits?.cast && details.credits.cast.length > 0 && (
            <div className="text-xs text-[#52525B] space-x-1 font-mono">
              <span className="text-[#09090B] font-bold uppercase text-[10px]">Cast:</span>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-black/[0.08]">
            <div className="flex items-center gap-3">
              <button
                ...
                className="px-6 py-2 rounded-full bg-[#2563EB] hover:bg-[#1D4ED8] text-white font-semibold text-xs uppercase tracking-wider flex items-center gap-2 transition cursor-pointer shadow-md"
              >
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full border text-xs font-medium uppercase tracking-wider flex items-center gap-2 transition cursor-pointer ${
                  isSaved 
                    ...
                    : 'bg-black/[0.04] border-black/10 text-[#52525B] hover:text-[#09090B] hover:bg-black/[0.08]'
                }`}
              >
            ...
            <button
              ...
              className="text-xs text-[#52525B] hover:text-[#2563EB] flex items-center gap-1.5 font-mono group py-1.5 cursor-pointer transition font-medium"
            >
