...
export default function QuickViewModal({ media, type = 'movie', isOpen, onClose }) {
  ...
  const handleToggleList = (e) => {
    e.stopPropagation();
    storageService.togglePlaylistItem({ ...currentData, media_type: mediaType });
    setIsSaved(!isSaved);
  };
  ...
  return (
    ...
    >
      <div 
        className="relative w-full max-w-2xl bg-[#0E1017] border border-white/[0.08] rounded-2xl overflow-hidden shadow-2xl animate-slide-up"
      ...
      >
        ...
        <div className="relative h-60 sm:h-72 w-full bg-black overflow-hidden flex items-center justify-center">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-[#0E1017] via-[#0E1017]/40 to-transparent pointer-events-none" />

          {/* Sound Toggle */}
          {trailerKey && (
            <button
              onClick={() => setIsMuted(!isMuted)}
              className="absolute bottom-4 right-4 z-30 p-2 rounded-full bg-black/70 hover:bg-black text-zinc-300 hover:text-white border border-white/15 backdrop-blur-md transition cursor-pointer"
          ...
          <div className="absolute top-4 left-4 z-20">
            <span className="px-2.5 py-0.5 rounded border border-white/15 bg-black/70 backdrop-blur-md text-white text-[10px] font-mono uppercase tracking-wider">
              {mediaType}
            </span>
          </div>
        </div>
        ...
        <div className="p-6 sm:p-7 space-y-5">
              ...
              {currentData.vote_average > 0 && (
                <>
                  <span className="flex items-center gap-1 text-zinc-200">
                    <Star className="w-3 h-3 text-zinc-400 stroke-[1.5]" /> {currentData.vote_average.toFixed(1)}
                  </span>
                  <span>·</span>
                </>
              )}
            ...
            {currentData.genres && currentData.genres.length > 0 && (
              <div className="flex flex-wrap gap-1.5 pt-1">
                {currentData.genres.slice(0, 4).map((g) => (
                  <span key={g.id} className="px-2 py-0.5 rounded border border-white/[0.08] text-[10px] font-mono text-zinc-400">
                    {g.name}
                  </span>
                ))}
              </div>
          ...
          <p className="text-zinc-300 text-xs sm:text-sm leading-relaxed line-clamp-3 font-light">
          ...
          {details?.credits?.cast && details.credits.cast.length > 0 && (
            <div className="text-xs text-zinc-400 space-x-1 font-mono">
              <span className="text-zinc-500 uppercase text-[10px]">Cast:</span>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-white/[0.06]">
            <div className="flex items-center gap-3">
              <button
                ...
                className="px-6 py-2 rounded-full bg-white hover:bg-zinc-200 text-black font-semibold text-xs uppercase tracking-wider flex items-center gap-2 transition cursor-pointer"
              >
            ...
            <button
              ...
              className="text-xs text-zinc-400 hover:text-white flex items-center gap-1.5 font-mono group py-1.5 cursor-pointer transition"
            >
