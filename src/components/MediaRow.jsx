    ...
    <section className="space-y-3 px-6 md:px-12 my-10 max-w-[1440px] mx-auto content-auto select-none">
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          {subtitle && (
            <p className="text-[11px] text-zinc-400 tracking-wide mt-0.5">{subtitle}</p>
          )}
        </div>
                ...
                <div className="relative aspect-[2/3] w-full rounded-none overflow-hidden bg-[#121212] border border-white/[0.08] group-hover/item:border-white/40 transition-all duration-200 group-hover/item:scale-[1.02] shadow-sm hover:shadow-md">
                  {posterImg ? (
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center p-3 text-center text-zinc-500 bg-[#121212]">
                      ...
                      <span className="text-[9px] text-zinc-500 line-clamp-2">{item.title || item.name}</span>
                    </div>
                  )}
                  
                  {/* Top Left: Top 10 Rank Badge or Media Type Badge */}
                  <div className="absolute top-2 left-2 z-20 pointer-events-none">
                    {isTop10 ? (
                      <span className="px-2 py-0.5 rounded-none bg-black/90 text-white text-[9px] font-bold tracking-wider shadow-sm border border-white/10">
                        #{rank}
                      </span>
                    ) : (
                      <span className="px-2 py-0.5 rounded-none bg-black/90 text-white text-[9px] font-bold uppercase tracking-wider shadow-sm border border-white/10">
