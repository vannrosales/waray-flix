...
export default function MediaCard({
  ...
  return (
    ...
    >
      {/* Poster Card Canvas - Sharp Geometric with No Border Radius */}
      <div className={`relative ${aspect} w-full rounded-none overflow-hidden bg-[#121212] border border-white/[0.08] group-hover/item:border-white/40 transition-all duration-200 group-hover/item:scale-[1.02] shadow-sm hover:shadow-md`}>
        {poster ? (
          <img
            src={poster}
            alt={title}
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center p-3 text-center text-zinc-500 bg-[#121212]">
            ...
            <span className="text-[9px] text-zinc-500 line-clamp-2">{title}</span>
          </div>
        )}
        ...
        <div className="absolute top-2 inset-x-2 flex items-center justify-between pointer-events-none z-20">
          {customBadge && (
            <span className="bg-black/90 text-white text-[9px] font-bold px-2 py-0.5 rounded-none shadow-sm border border-white/10">
              {customBadge}
            </span>
          )}

          {showRating && rating && (
            <div className="ml-auto bg-black/80 backdrop-blur-md px-2 py-0.5 rounded-none flex items-center gap-1 border border-white/15 shadow-sm text-white">
        ...
        {onQuickView && (
          <div className="absolute inset-0 bg-black/50 opacity-0 group-hover/item:opacity-100 transition-opacity duration-200 flex items-center justify-center pointer-events-none">
            <button
              ...
              className="pointer-events-auto flex items-center gap-1.5 px-3 py-1.5 rounded-none bg-white text-black text-[11px] font-bold shadow-lg hover:scale-105 transition"
            >
