...
export default function Movies() {
  useDocumentTitle('WarayFlix — Movies');
  ...
  useEffect(() => {
    async function loadInitialMovies() {
      try {
        ...
        const res = await fetch(`${BASE_URL}/discover/movie?api_key=${API_KEY}&sort_by=popularity.desc${genreQuery}&page=1`);
  ...
  const loadMoreMovies = async () => {
    ...
    try {
      ...
      const genreQuery = activeGenre === 'all' ? '' : `&with_genres=${activeGenre}`;
      const res = await fetch(`${BASE_URL}/discover/movie?api_key=${API_KEY}&sort_by=popularity.desc${genreQuery}&page=${nextPage}`);
      const data = await res.json();
  ...
  return (
    <div className="min-h-screen bg-[#0B0D10] text-white selection:bg-white selection:text-black font-sans">
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-16 py-16 space-y-12">
        ...
        {/* Movies Grid matching your row card design */}
        ...
        {page < totalPages && (
          <div className="flex justify-center pt-8">
            ...
            >
              <span>{loadingMore ? 'LOADING_MORE...' : 'LOAD MORE MOVIES'}</span>
